...
_oDragSelf._moImgDragUpload=_oTop.QMFileUpload.create('drag',{bNoStyle:true,oContainer:_oDragSelf._moCfg.oImgUploadArea,sDragLeaveMsg:'<div style="'+_sCss+'">\u5C06\u56FE\u7247\u62D6\u62FD\u81F3\u6B64\u533A\u57DF\uFF0C\u53EF\u76F4\u63A5\u6DFB\u52A0\u81F3\u6B63\u6587\u3002</div>',sDragEnterMsg:'<div style="'+_sCss+'">\u91CA\u653E\u9F20\u6807</div>',oComlist:["FlashH5DragMail","Html5DragMail"],onproxyclose:_oDragSelf._onProxyClose(),onselect:function(_aoFiles){
_oDragSelf._hideDragAndDropArea();
...
_oTop.loadJsFileToTop(["$js_path$webp/qmplayer/player289cd9.js","$js_path$webp/com/kits/qmpreviewer/js/qmpreviewer289d4d.js"]);
_oTop.waitFor(function(){
...
_oDiv.id="D"+_sName;
_oDiv.innerHTML=this._getFileCellTmpl(_aoFile.get("sUploadMode")).replace({id:_aoFile.get("sId"),name:_sName,value:_aoFile.get("sName"),images_path:_oTop.getPath("image",true),ext:_aoFile.get("sUploadMode"),filename:_oTop.htmlEncode(_aoFile.get("sName")),dispfname:_insertWbr(_aoFile.get("sName")),size:_aoFile.get("nSize")||0,formatsize:this._formatSize(parseInt(_aoFile.get("nSize")||0)),mailattach:_aoFile.get("sUploadMode")=="collection"?[_aoFile.mailid,_aoFile.attachid,_aoFile.attachname].join("|"):"",ftn:_aoFile.get("bUpToFtn"),bResumeAttach:_aoFile.get('bResumeAttach')});
...
if((window.File&&window.XMLHttpRequest&&(new XMLHttpRequest()).upload&&!_oTop.gbIsEdge)||_oTop.detectActiveX(4,1))
{
...
if(_oSelf.getAttachSize()*1.3+_nTmpTotalSize>_oSelf.getAttachLimit())
{
...
if(_abIsFtn)
{
var _bGoFtn=getTop().detectActiveX(3)||getTop().detectActiveX(5);
if(_aoFiles[0].oUploader)
{
var _sUploaderName=_aoFiles[0].oUploader.name;
if(/FlashPopup/.test(_sUploaderName))
{
...
var _sFileSize=new RegExp("\\((.+)\\)","ig").exec(_oSpanChildNodes[_oSpanChildNodes.length-1].innerHTML)[1],_sMode=_oTop.attr(os[i],"mode"),_nAppId=_oTop.attr(os[i],"appid"),_sExpiretimeStr=_nExpiretime>0?_oTop.formatDate(new Date(_nExpiretime*1000),"$YY$\u5E74$MM$\u6708$DD$\u65E5 $hh$:$mm$"):'';
...
{
var _oWin=_oCompose.getWin(),_bIsText=((_oEditor&&_oEditor.getContentType&&_oEditor.getContentType())=="text"||false),_bIsSend=(_asTemplate===""&&_asAction==="send");
...
_nStep++;
if(_anStartStep<_nStep&&_oCompose.getPageId()!='card'&&_oCompose.getPageId()!='voice'&&_oCompose.getPageId()!='postcard')
{
var _oAttachExistList=_oCompose.moAttach.getExistList(),_oBigAttachExistList=_oProcess._getBigAttachNameList(["ResumeWorks","BigAttach","exist_BigAttach"]),_bIsHasShowResumeConfirm=_oTop.getCookie("showResumeConfirmFlag")=="1"||(_oProcess.S("ipt_showResumeConfirmFlag")&&(_oProcess.S("ipt_showResumeConfirmFlag").value!="false")),_sSubject=_oTop.isShow(_oProcess.S("subject"))?_oProcess.S("subject").value:"",_sContent=_oCompose.getPageEditor().getContent(true),_sAliasName=getUserInfo("alias"),_sName,_bIsPassChecked=true,_sResumePatt=/resume/ig;
...
if(_sSubject&&(_sSubject.indexOf("\u7B80\u5386")>-1||_sSubject.search(_sResumePatt)>-1))
{
_bIsPassChecked=false;
}
...
if(_sName&&(_sName.indexOf("\u7B80\u5386")>-1||_sName.search(_sResumePatt)>-1))
{
_bIsPassChecked=false;
return false;
}
...
if(_bIsPassChecked&&_sContent&&(_sContent.indexOf("\u7B80\u5386")>-1||_sContent.search(_sResumePatt)>-1))
{
_bIsPassChecked=false;
}
if(!_bIsPassChecked)
...
return true;
},_saveResumeConfirmed:function(){
var _oProcess=this,_oTop=getTop();
_oTop.QMAjax.send("/cgi-bin/do",{content:"action=resumeconfirm",method:"POST",onload:function(_abIsOk){
if(_abIsOk)
{
var _nDate=new Date();
_nDate.setDate(_nDate.getDate()+7);
_oTop.setCookie("showResumeConfirmFlag",'1',_nDate);
(_oProcess.S("ipt_showResumeConfirmFlag")&&(_oProcess.S("ipt_showResumeConfirmFlag").value=true));
}
}});
...
var _oTop=getTop();
_oTop.localReloadJsFile("$js_path$webp/libcompose24e6ae.js",function(){
...
{
_oTop.loadJsFileToTop(["$js_path$webp/com/kits/qmeditor/qqmail/release/editor29b6d5.js"]);
...
{
_oTop.loadJsFileToTop(["$js_path$webp/backsend24e6ae.js"]);
}
_oTop.loadJsFileToTop(["$js_path$webp/composeresp299406.js"]);
...
var _oAttach=_oCompose.moAttach;
var _oTop=getTop();
_oWin.addBigAttach=function(_asExpireTime,_asFileName,_asSize,_asUrl,_asCode,_asFid){
...
_oCompose._addExistBigAttach([{id:_asFid,filesize:_asSize,filename:_asFileName,downloadlink:_asUrl,code:_asCode,key:_asUrl.replace(/^(.*?\?k=)([0-9a-zA-Z]+)(.*)?$/gi,"$2"),fid:_asFid,exptime:_asExpireTime}]);
_oCompose.setEditedAttach(true);
}
};
...
if(!!_oReloadInfo)
{
var _sBigAttachCode=_oReloadInfo.bacode,_sExistFileCode=_oReloadInfo.excode,_bHideBigAttach=_aoConfig&&_aoConfig.hideBigAttach,_oFileUploadData=_oReloadInfo.fileuploaddata,_oSubjectDom=_oCompose.S("subject"),_oBigAttachDom=_oCompose.S("BigAttach"),_oExistFileDom=_oCompose.S("exist_file"),_oEditor=_oCompose.getPageEditor();
...
var _oInfo={content:_oCompose.getPageEditor()&&_oCompose.getPageEditor().getContent(),subject:_oCompose.S("subject").value,exbacode:_oCompose.S("exist_BigAttach")&&_oCompose.S("exist_BigAttach").innerHTML,bacode:_sBacode,excode:_sExcode,fileuploaddata:_oFileUploadData||[],hasResume:_oResumeInput&&_oResumeInput.value?true:false,resumesender:_oCompose.getResumeSender_()};
...
setTimeout(function(){
_oTop.gbIsChrome&&(_oTop.detectActiveX(5,1));
},0);
